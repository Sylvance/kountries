  ...
  module Africa
    def self.find_country_by_name(name)
      constants.each do |country|
        african_country_class = "Kountries::Africa::#{country}".constantize
    ...
    def self.find_country_by_code(code)
      return empty_struct unless Kountries::Africa.constants.include?(code.to_sym)

      african_country_class = "Kountries::Africa::#{code}".constantize
    ...
    class ST
      ...
      UN_OFFICIAL_NAMES = ['São Tomé and Príncipe', 'São Tomé und Príncipe', 'São Tomé et Príncipe', 'Santo Tomé y Príncipe', 'サントメ・プリンシペ',
    ...
    class LY
      ...
      COUNTRY_CODE = '218'
      NAME = 'Libya'
      UN_OFFICIAL_NAMES = ['Libya', 'ليبيا', 'Libyen', 'Libye', 'Libia', 'リビア', 'Libië', 'Libyan Arab Jamahiriya'].freeze
    end
    ...
    class EH
      ...
      COUNTRY_CODE = '212'
      NAME = 'Western Sahara'
      UN_OFFICIAL_NAMES = ['Western Sahara', 'الصحراء الغربية', 'Westsahara', 'Sahara Occidental', '西サハラ', 'Westelijke Sahara'].freeze
    end
    ...
    class CD
      ...
      UN_OFFICIAL_NAMES = ['Congo (Dem. Rep.)', 'Kongo (Dem. Rep.)', 'Congo (Rep. Dem.)', 'コンゴ民主共和国', 'Congo [DRC]',
                           'Congo (The Democratic Republic Of The)', 'Democratic Republic of the Congo', 'Congo, Democratic Republic of'].freeze
    end
    ...
    class ZA
      ...
      COUNTRY_CODE = '27'
      NAME = 'South Africa'
      UN_OFFICIAL_NAMES = ['South Africa', 'Republik Südafrika', 'Afrique du Sud', 'República de Sudáfrica', '南アフリカ', 'Zuid-Afrika'].freeze
    end
    ...
    class SH
      ...
      UN_OFFICIAL_NAMES = ['Saint Helena', 'Sankt Helena', 'Sainte Hélène', 'Santa Helena', 'セントヘレナ・アセンションおよびトリスタンダクーニャ', 'Sint-Helena',
